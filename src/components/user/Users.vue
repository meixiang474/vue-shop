          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                size="mini"
                @click="openSetRolesDialog(scope.row)"
              />
            </el-tooltip>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 打开分配角色对话框
    async openSetRolesDialog (user) {
      const { data: res } = await this.$http.get('roles')
      if (res.meta.status !== 200) return this.$message.error('获取角色列表失败')
      this.rolesList = res.data
      this.setRolesDialogVisible = true
      this.currentUserName = user.username
      this.currentUserRole = user.role_name
      this.currentUserId = user.id
    },
    // 提交选择了的角色
    async submitSelectedRole () {
      if (!this.selectedRole) return this.$message.error('请选择要分配的角色')
      const { data: res } = await this.$http.put(`users/${this.currentUserId}/role`, {
